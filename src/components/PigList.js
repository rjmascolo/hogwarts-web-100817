...
export default class PigList extends React.Component {
  ...
  render() {
    const pigGreasy = this.props.pigList.filter(pig => this.props.isGreasy ? pig.greased : pig)
    const sortedBy = this.props.filterBy === "name" ? this.sortByName(pigGreasy) : this.sortByWeight(pigGreasy);
